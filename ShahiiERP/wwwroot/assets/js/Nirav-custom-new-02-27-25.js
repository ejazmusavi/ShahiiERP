...
paginationDropdownItems.forEach(item => {
    item.addEventListener('click', function(e) {
        ...
        dropdownButton.textContent = "Page " + perPage;
    });
});
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const headerCheckbox = document.querySelector('#myTable #checkebox-md');
...
function getDropdownValue(event,tableId){
...
document.addEventListener("DOMContentLoaded", function () {
  // Select all tables that need sorting capability
  const tables = document.querySelectorAll(".table"); // Add this class to all your sortable tables
  
  tables.forEach(table => {
    const theads = table.getElementsByTagName("thead");
    // For each table, find its headers - adjust the selector to match your table structure
    // This assumes your first thead contains the main headers you want to sort by
    const headerRow = table.querySelector("thead tr");
    const headers = Array.from(headerRow.querySelectorAll("th")).slice(2); // Skip first two columns
    
    // Create separate click counters for each table
    const clickCounters = Array(headers.length).fill(0);
    
    // Store original rows for each table separately
    ...
    // Add click event listeners to headers for this specific table
    headers.forEach((header, index) => {
      header.addEventListener("click", function() {
        // Sort this specific table
        sortTable(table, index + 2, index, clickCounters, originalRows);
      });
    });
  });
  
  // Modified sort function that works on a specific table
  function sortTable(table, colIndex, headerIndex, clickCounters, originalRows) {
    const tbody = table.querySelector("tbody");
    const rows = Array.from(tbody.rows);
    const headerRow = table.querySelector("thead tr");
    const headers = Array.from(headerRow.querySelectorAll("th")).slice(2);
    const currentHeader = headers[headerIndex];
    
    // Reset display of all arrows for this table
    table.querySelectorAll("th i.fa-solid").forEach(icon => {
      icon.style.display = 'none';
    });
    
    // Show the arrow for the current header
    const arrow = currentHeader.querySelector("i.fa-solid");
    arrow.style.display = 'inline-block';
    
    // Increment click counter for this column
    clickCounters[headerIndex]++;
    
    // Determine sort direction
    const isAscending = clickCounters[headerIndex] % 2 === 1;
    ...
    if (clickCounters[headerIndex] === 3) {
      ...
      rows.sort((a, b) => {
        ...
        // Add date detection logic here if required
